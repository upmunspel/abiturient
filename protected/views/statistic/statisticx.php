...
//Yii::app()->db->get
$connect_status = mysql_connect(/*"localhost","root","parol","abiturient"*/"10.1.10.26","edbo","eU7InIl","abiturient");
if (!$connect_status){
	echo "<center>
		...
		if ($count_specializations){
			for ($l = 0; $l < $count_specializations; $l++){
                                ...
                                for ($date_index = 0; $date_index < $dates_count; $date_index++){
                                    ...
                                    $table[$s] .= "<td>".$count['B_dnevn_per_day']."</td><td style='border-right:2px solid black;'>".
                                            $count['B_zaoch_per_day']."</td>";
                                }
                                $table[$s] .= "<td style='font-weight: bold;'>".$general_count_dnevn."</td><td style='border-right:2px solid black;font-weight: bold;'>".$general_count_zaoch."</td></tr>";
                                $s++;
			}
		} else {
			if ($j > 0){
...
for ($date_index = 0; $date_index < $dates_count; $date_index++){
?>
    <th colspan="2" align='center' style="font-size:10pt;border-right:2px solid black;">За <br/><?php echo $cday[$date_index];?>.07.2013</th>
<?php } ?>
    <th colspan="2">Загалом</th>
</tr>
<tr>
<?php
for ($date_index = 0; $date_index < $dates_count+1; $date_index++){
?>
    <th align='center' style="font-size:10pt;">Денна<br/>кількість</th>
    <th align='center' style="font-size:10pt;border-right:2px solid black;">Заочна<br/>кількість</th>
<?php } ?>
</tr>
